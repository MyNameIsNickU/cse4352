...
// starts at first variable, counts till checksum filed, skip to sourceIp
// 
void etherCalcIpChecksum(ipHeader* ip)
{
	...
	/*
	ip->headerChecksum = 0;
	etherSumWords(ip, ((ip->revSize & 0xF) * 4) - 12, &sum);
	*/
...
void etherSendPingResponse(etherHeader *ether)
{
	...
	/*
	char str[10];
	putsUart0("-Ping Response-\n");
	putsUart0("From: ");
	for(i = 0; i < IP_ADD_LENGTH; i++)
	{
		sprintf(str, "%u", ip->sourceIp[i]);
		putsUart0(str);
		if (i < 4-1)
            putcUart0('.');
	}
...
bool etherIsUdp(etherHeader *ether)
{
    ...
    uint8_t optionsP[20] = dhcp->options;
    uint8_t i;

    /*if(dhcp->op == 2)
    {
        putsUart0("State is: ");
        putcUart0(dhcpGetState() + 48);
        putcUart0('\n');
        for(i = 0; i < 20; i++)
            optionsP[i] = dhcp->options[i];
        ok = ok;
    }*/
...
void etherSendUdpResponse(etherHeader *ether, uint8_t *udpData, uint8_t udpSize)
{
    ipHeader *ip = (ipHeader*)ether->data;
    uint8_t ipHeaderLength = (ip->revSize & 0xF) * 4;
    udpHeader *udp = (udpHeader*)((uint8_t*)ip + ipHeaderLength); // casting as uint8_t allows byte address incrementing
    uint16_t udpLength;
    uint8_t *copyData;
    uint32_t sum = 0;
    uint8_t i, tmp8;
