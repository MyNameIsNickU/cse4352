...
{
  uint8_t op; // 1 for request, 0 for reply
  uint8_t htype;
  uint8_t hlen;
  uint8_t hops;
  uint32_t  xid;
  uint16_t secs;
  uint16_t flags;
  uint8_t ciaddr[4]; // current ip filled if BOUND, RENEW, REBINDING
  uint8_t yiaddr[4]; //
  uint8_t siaddr[4]; // server IP giving us IP
  uint8_t giaddr[4]; // 
  uint8_t chaddr[16]; // first 6 bytes, rest 0s
  uint8_t data[192]; // sname + file
  uint32_t magicCookie; // part of options[0] "DHCP or 99.130.83.99"
  uint8_t options[0];
} dhcpFrame;
